...
// let twitterSocket = new Socket("/socket", {params: {token: window.userToken}})
...
export var TwitterStream = {
  run: function() {
    let twitterSocket = new Socket("/socket", {params: {token: window.userToken}})

    twitterSocket.connect()
    // Now that you are connected, you can join channels with a topic:
    let channel = twitterSocket.channel("room:lobby", {})
    let id = 0
    ...
    channel.on("tweet", payload => {
      ...
      // NOTE: Extract this to a sepate function, like renderTweet()
      let newline = /\n|\r/g
