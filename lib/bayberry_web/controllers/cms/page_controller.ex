defmodule BayberryWeb.Blog.PageController do
  ...
  def create(conn, %{"page" => page_params}) do
    case Blog.create_page(conn.assigns.current_author, page_params) do
      {:ok, page} ->
        conn
        |> put_flash(:info, "Page created successfully.")
        |> redirect(to: blog_page_path(conn, :show, page))
  ...
  def edit(conn, _) do
    changeset = Blog.change_page(conn.assigns.page)
    render(conn, "edit.html", changeset: changeset)
  end

  def update(conn, %{"page" => page_params}) do
    case Blog.update_page(conn.assigns.page, page_params) do
      {:ok, page} ->
        conn
  ...
  def delete(conn, _) do
    {:ok, _page} = Blog.delete_page(conn.assigns.page)

    conn
    |> put_flash(:info, "Page deleted successfully.")
    |> redirect(to: blog_page_path(conn, :index))
  end
  ...
  defp authorize_page(conn, _) do
    page = Blog.get_page!(conn.params["id"])
